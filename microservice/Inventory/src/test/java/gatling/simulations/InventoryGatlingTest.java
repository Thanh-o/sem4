...
public class InventoryGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8082");
        ...
        .on(
            exec(
                http("Get all inventories")
                    .get("/services/inventory/api/inventories")
                ...
                .exec(
                    http("Create new inventory")
                        .post("/services/inventory/api/inventories")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"warehouse\": \"SAMPLE_TEXT\"" + ", \"productId\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .on(
                    exec(
                        http("Get created inventory").get("/services/inventory${new_inventory_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created inventory").delete("/services/inventory${new_inventory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
