 ...
 */
public class ProductGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all products").get("/services/product/api/products").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created product").get("/services/product${new_product_url}").headers(headersHttpAuthenticated)).pause(10)
                )
                .exec(http("Delete created product").delete("/services/product${new_product_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Product entity").exec(scn);
