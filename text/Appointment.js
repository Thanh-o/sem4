...
const Appointment = () => {
  ...
  const [availableDoctors, setAvailableDoctors] = useState([]); // Doctors available for selected time slot
  ...
  useEffect(() => {
    if (formData.date) {
      ...
      // Set all time slots as available initially
      setAvailableSlots(timeSlots.filter(slot => 
  ...
  }, [formData.timeSlot, doctors, formData.date]);
  ...
  const filterAvailableDoctors = async (doctorList, date, timeSlot) => {
    ...
    try {
      ...
      // Filter out doctors who are already booked for the selected date and time
      const availableDoctorIds = doctorList
        .filter(doctor => {
      ...
      // Also check for locked slots
      const lockedSlotsPromises = doctorList.map(doctor =>
        axios.get(`http://localhost:8081/api/v1/appointments/check-locked-slots?doctorId=${doctor.doctor_id}&date=${date}`)
          .then(response => ({
            doctorId: doctor.doctor_id,
            lockedSlots: response.data
          }))
          .catch(error => ({
            doctorId: doctor.doctor_id,
            lockedSlots: []
      ...
      const lockedSlotsData = await Promise.all(lockedSlotsPromises);
      
      // Filter out doctors with locked slots
      const finalAvailableDoctors = doctorList.filter(doctor => {
        const isNotBooked = availableDoctorIds.includes(doctor.doctor_id);
        const lockedSlots = lockedSlotsData.find(
          data => data.doctorId === doctor.doctor_id
        )?.lockedSlots || [];
        const isNotLocked = !lockedSlots.includes(timeSlot);
        
        return isNotBooked && isNotLocked;
      });
  ...
  const handleDoctorSelect = async (doctor) => {
    // Lock the slot before selecting the doctor
    try {
      await axios.post("http://localhost:8081/api/v1/appointments/lock-slot", {
        doctorId: doctor.doctor_id,
        date: formData.date,
      ...
      // Unlock the slot after 5 minutes
      setTimeout(() => {
        axios
          .post("http://localhost:8081/api/v1/appointments/unlock-slot", {
            doctorId: doctor.doctor_id,
            date: formData.date,
            time: formData.timeSlot,
          })
      ...
      $(".doctor-list").append(
        '<span class="time-error">This time slot is already taken for this doctor, please choose another one.</span>'
      );
      setTimeout(function () {
        $(".time-error").remove();
      }, 3000);
  ...
  const renderDoctorsList = () => {
    ...
    return (
      <div className="doctor-list">
        <label>Doctors Available for Selected Time</label>
                  ...
                  <div>
                    ...
                    {selectedDepartment && (
                      <div className="department-info">
                        ...
                        <div>
                          <img
                            src={selectedDepartment.department_img}
                            alt="Department"
                          />
